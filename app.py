...
with st.sidebar:
    ...
    <div style="padding: 0.5rem 0 1.25rem 0;">
        <h2 style="color:#2C3E50; font-size:1.15rem; font-weight:700; margin-bottom:0.5rem;">
            🩺 About this app
        </h2>
        <p style="font-size:0.82rem; color:#3D5166; line-height:1.7;">
            This assistant demonstrates how a machine learning model reasons
            through symptoms to rank possible diagnoses — one question at a time.
            At each step, the model asks the single most informative follow-up
            question based on what it has learned so far.
        </p>
        <p style="font-size:0.82rem; color:#3D5166; line-height:1.7; margin-top:0.5rem;">
            It is built on the <strong>DDXPlus</strong> dataset — 1.3 million
            synthetic patients across 49 conditions — created by researchers at
            <strong>Mila-Quebec AI Institute</strong>, using a clinician-designed
            diagnostic knowledge base and Dialogue Health Technologies' rule-based
            diagnosis engine.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("⚠️ Limitations & Disclaimer", expanded=False):
        st.markdown("""
        <ul style="font-size:0.78rem; color:#5F6F7E; line-height:1.9; padding-left:1.1rem;">
            <li>The dataset is <strong>fully synthetic</strong> — not derived from real patient records</li>
            <li>Results have <strong>no clinical validity</strong> and must never inform any real medical decision</li>
            <li>The model reasons only over <strong>49 conditions</strong> and <strong>~223 symptoms</strong> in the dataset</li>
            <li>Probabilities reflect dataset statistics, <strong>not</strong> real-world disease prevalence</li>
        </ul>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <p style="font-size:0.76rem; color:#8A9BB0; line-height:1.7;">
        <strong>Citation:</strong><br>
        Fansi Tchango et al.,<br>
